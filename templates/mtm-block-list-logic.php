	...
	<div class="mtm-module--list">
		...
		if( _get_field( 'mtm_show_view_all_link' ) ) :
			...
			if( _get_field( 'mtm_view_all_link_text' ) ): ?>
				<a aria-label="View All" class="mtm-view-all-link" href="<?php echo get_term_link( $term, $taxonomy ); ?>"><?php _e( _get_field( 'mtm_view_all_link_text' ), 'mtm' ); ?></a>
			<?php else: ?>
				<a aria-label="View All" class="mtm-view-all-link" href="<?php echo get_term_link( $term, $taxonomy ); ?>"><?php _e( $viewtext, 'mtm' ); ?></a>
			<?php endif;
		endif;?>
	</div>
	...
	<div class="mtm-module--list">
		...
		if( _get_field( 'mtm_show_view_all_link' ) ) :
			...
			if( 'post' == $posttype  ) : // Posts ?>
				<a aria-label="View All" class="mtm-view-all-link" href="<?php echo get_permalink( get_option('page_for_posts' ) ); ?>"><?php _e( $viewtext, 'mtm' ); ?></a>
			<?php elseif( 'page' == $posttype || 'attachment' == $posttype || 'revision' == $posttype || 'nav_menu_item' == $posttype ) : // Unsupported archives
				// Nothing
			else: // everything else ?>
				<a aria-label="View All" class="mtm-view-all-link" href="<?php echo get_site_url() . '/' . $posttype ?>"><?php _e( $viewtext, 'mtm' ); ?></a>
			<?php endif;
		endif; ?>
	</div>
